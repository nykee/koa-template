...
const update = async ctx => {
  const {id, title, author, summary, category, tag, content,description} = ctx.request.body
  const data =  await Article.update({id, title, author, summary,description, category, tag, content},
    {where: {id}}
  )
  ctx.body = {
    code: 200,
